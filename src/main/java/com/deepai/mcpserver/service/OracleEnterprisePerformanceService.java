...
public class OracleEnterprisePerformanceService {
    ...
    @Tool(name = "oracle_parallel_execution",
          description = "Manage parallel query execution for performance optimization")
    public Map<String, Object> manageParallelExecution(
        @ToolParam(name = "operation", required = true) String operation,
        @ToolParam(name = "sqlQuery", required = false) String sqlQuery,
        @ToolParam(name = "parallelDegree", required = false) Integer parallelDegree,
        @ToolParam(name = "tableName", required = false) String tableName) {
            ...
            switch (operation.toUpperCase()) {
                ...
                case "PARALLEL_STATUS":
                    ...
                    List<Map<String, Object>> parallelSessions = jdbcTemplate.queryForList(
                        "SELECT qcsid, server_name, status, degree FROM v\ " +
                        "ORDER BY qcsid"
                    );
    ...
    @Tool(name = "oracle_table_partitioning",
          description = "Manage table partitioning for performance and maintenance")
    public Map<String, Object> manageTablePartitioning(
        @ToolParam(name = "operation", required = true) String operation,
        @ToolParam(name = "tableName", required = true) String tableName,
        @ToolParam(name = "partitionType", required = false) String partitionType,
        @ToolParam(name = "partitionColumn", required = false) String partitionColumn,
        @ToolParam(name = "partitionName", required = false) String partitionName) {
    ...
    @Tool(name = "oracle_materialized_views",
          description = "Manage materialized views for query performance optimization")
    public Map<String, Object> manageMaterializedViews(
        @ToolParam(name = "operation", required = true) String operation,
        @ToolParam(name = "mvName", required = true) String mvName,
        @ToolParam(name = "baseQuery", required = false) String baseQuery,
        @ToolParam(name = "refreshType", required = false) String refreshType,
        @ToolParam(name = "refreshInterval", required = false) String refreshInterval) {
    ...
    @Tool(name = "oracle_query_optimization",
          description = "Advanced query optimization and hint management")
    public Map<String, Object> optimizeQuery(
        @ToolParam(name = "sqlQuery", required = true) String sqlQuery,
        @ToolParam(name = "optimizationType", required = false) String optimizationType,
        @ToolParam(name = "hints", required = false) List<String> hints,
        @ToolParam(name = "analyzeExecution", required = false) Boolean analyzeExecution) {
    ...
    @Tool(name = "oracle_memory_management",
          description = "Manage Oracle memory structures (SGA, PGA) for optimal performance")
    public Map<String, Object> manageMemory(
        @ToolParam(name = "operation", required = true) String operation,
        @ToolParam(name = "memoryType", required = false) String memoryType,
        @ToolParam(name = "targetSize", required = false) String targetSize) {
            ...
            switch (operation.toUpperCase()) {
                ...
                case "PGA_STATUS":
                    List<Map<String, Object>> pgaStats = jdbcTemplate.queryForList(
                        "SELECT name, value, unit FROM v\ ORDER BY name"
                    );
                    ...
                    List<Map<String, Object>> pgaWorkarea = jdbcTemplate.queryForList(
                        ...
                        "FROM v\ ORDER BY estimated_optimal_size DESC FETCH FIRST 10 ROWS ONLY"
                    );
                ...
                case "BUFFER_CACHE_ADVICE":
                    List<Map<String, Object>> bufferAdvice = jdbcTemplate.queryForList(
                        ...
                        "FROM v\ WHERE name = 'DEFAULT' ORDER BY size_for_estimate"
                    );
                ...
                case "SHARED_POOL_ADVICE":
                    List<Map<String, Object>> sharedPoolAdvice = jdbcTemplate.queryForList(
                        ...
                        "FROM v\ ORDER BY shared_pool_size_for_estimate"
                    );
                ...
                case "PGA_ADVICE":
                    List<Map<String, Object>> pgaAdvice = jdbcTemplate.queryForList(
                        ...
                        "FROM v\ ORDER BY pga_target_for_estimate"
                    );
    ...
    @Tool(name = "oracle_automatic_workload_repository",
          description = "Manage AWR snapshots and performance reporting")
    public Map<String, Object> manageAwrReports(
        @ToolParam(name = "operation", required = true) String operation,
        @ToolParam(name = "beginSnapId", required = false) Integer beginSnapId,
        @ToolParam(name = "endSnapId", required = false) Integer endSnapId,
        @ToolParam(name = "reportType", required = false) String reportType,
        @ToolParam(name = "instanceId", required = false) Integer instanceId) {
    ...
    @Tool(name = "oracle_sql_plan_management",
          description = "Manage SQL Plan Baselines for query execution stability")
    public Map<String, Object> manageSqlPlanBaselines(
        @ToolParam(name = "operation", required = true) String operation,
        @ToolParam(name = "sqlHandle", required = false) String sqlHandle,
        @ToolParam(name = "planName", required = false) String planName,
        @ToolParam(name = "sqlText", required = false) String sqlText) {
    ...
    @Tool(name = "oracle_compression_management",
          description = "Manage table and index compression for storage optimization")
    public Map<String, Object> manageCompression(
        ...
        @ToolParam(name = "compressionType", required = false) String compressionType) {
    ...
    @Tool(name = "oracle_resource_manager",
          description = "Manage Database Resource Manager for workload control")
    public Map<String, Object> manageResourceManager(
        @ToolParam(name = "operation", required = true) String operation,
        @ToolParam(name = "planName", required = false) String planName,
        @ToolParam(name = "groupName", required = false) String groupName,
        @ToolParam(name = "cpuP1", required = false) Integer cpuP1,
        @ToolParam(name = "cpuP2", required = false) Integer cpuP2) {
            ...
            switch (operation.toUpperCase()) {
                ...
                case "RESOURCE_STATISTICS":
                    List<Map<String, Object>> resourceStats = jdbcTemplate.queryForList(
                        ...
                        "consumed_cpu_time, yields FROM v\ " +
    ...
    @Tool(name = "oracle_io_optimization",
          description = "Optimize Oracle I/O performance and storage management")
    public Map<String, Object> optimizeIoPerformance(
        @ToolParam(name = "operation", required = true) String operation,
        @ToolParam(name = "tableName", required = false) String tableName,
        @ToolParam(name = "tablespaceName", required = false) String tablespaceName) {
            ...
            switch (operation.toUpperCase()) {
                ...
                case "SEGMENT_IO_ANALYSIS":
                    if (tableName != null) {
                        ...
                        List<Map<String, Object>> topSegments = jdbcTemplate.queryForList(
                            "SELECT object_name, logical_reads, physical_reads, " +
                            "physical_writes, buffer_busy_waits " +
                            "FROM v\ ORDER BY logical_reads DESC " +
                ...
                case "TEMP_SPACE_OPTIMIZATION":
                    List<Map<String, Object>> tempUsage = jdbcTemplate.queryForList(
                        ...
                        "FROM v\ ORDER BY used_blocks DESC"
                    );
